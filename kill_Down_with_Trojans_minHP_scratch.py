...
def DP(n, H, tile_types, tile_values):
    ...
    #print(memo)                  # COMMENT OUT!!!
    #TODO: print the memo in a more readable manner, or maybe if u flip the dimensiosn around (but that makes the code harder to read) 
    #print(memo.transpose())
    print(memo.transpose((3, 2, 0, 1)))  #might need to flip 3 and 2
    #print(memo.transpose((0, 1, 2, 3)))  #same as not tranposing lol, cuz transpose default has to be (3, 2, 1, 0) aka dimensions in reverse
    print("Starting hp:", H)
    #print("temp:", temp)
...
def DP_helper(memo, n, tile_types, tile_values, x, y, pTok, mTok):  #add tokens later
    if x == n-1 and y == n-1:
        ...
        return 0
    if x >= n or y >= n:
        return 100000000000000         # maybe this is the problem
